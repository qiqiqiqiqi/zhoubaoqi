...
public class KitkatCamera implements ICamera {
    ...
    public boolean open(int cameraId) {
        try {
            ...
            if (mCamera != null) {
                ...
                Log.e(TAG, "open():camera previewSize:" + mPreSize.x + "/" + mPreSize.y + ",camera pictureSize:" + mPicSize.x + "/" + mPicSize.y);
                return true;
            }
    ...
    private Camera.Size getPropPreviewSize(List<Camera.Size> list, float th, int minWidth) {
        ...
        for (Camera.Size s : list) {
            stringBuffer.append(s.width).append("/").append(s.height).append(",");
            if ((s.height >= minWidth) && equalRate(s, th)) {
                break;
            }
            i++;
        }
        Log.d(TAG, "getPropPreviewSize():list=" + stringBuffer.toString());
        if (i == list.size()) {
            i = 0;
        }
        return list.get(i);
    ...
    private Camera.Size getPropPictureSize(List<Camera.Size> list, float th, int minWidth) {
        ...
        Log.d(TAG, "getPropPictureSize():list=" + stringBuffer.toString());
        if (i == list.size()) {
            i = 0;
        }
        return list.get(i);
