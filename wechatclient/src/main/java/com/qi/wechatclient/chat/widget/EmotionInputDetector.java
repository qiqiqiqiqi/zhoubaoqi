package com.qi.wechatclient.chat.widget;
...
import com.qi.wechatclient.chat.util.AudioRecoderUtils;
import com.qi.wechatclient.chat.util.Constants;
import com.qi.wechatclient.chat.util.PopupWindowFactory;
import com.qi.wechatclient.chat.util.Utils;
import com.qi.wechatclient.dao.MessageInfoDao;
import com.qi.wechatclient.utils.UUIDUtils;
...
public class EmotionInputDetector {
    ...
    public EmotionInputDetector bindToFriend(Friend friend) {
        mFriend = friend;
        return this;
    }
    ...
    public EmotionInputDetector bindToSendButton(View sendButton) {
        ...
        sendButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String messageInfoID = UUIDUtils.createUUID();
                while (MessageInfoDao.getMessageInfoDaoInstance().queryMessageInfoByMessageInfoID(messageInfoID) != null) {
                    messageInfoID = UUIDUtils.createUUID();
                }
                messageInfo.setMessageInfoID(messageInfoID);
                messageInfo.setUserID(UserCache.getAppUserId());
                messageInfo.setFriendID(mFriend.getFriendID());
                messageInfo.setContent(mEditText.getText().toString());
                messageInfo.setType(Constants.CHAT_ITEM_TYPE_RIGHT);
                messageInfo.setSendState(Constants.CHAT_ITEM_SENDING);
                messageInfo.setCreateTime(System.currentTimeMillis()+"");
                MessageInfoDao.getMessageInfoDaoInstance().insertMessageInfo(messageInfo);
                // EventBus.getDefault().post(messageInfo);
                if (mOnSendMessageListener != null) {
                    mOnSendMessageListener.onSendMessage(messageInfo);
                }
                mEditText.setText("");
            }
        });
